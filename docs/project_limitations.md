...
The dissertation event-study design improves transparency by using pre-specified event dates, event-asset links, and sector-adjusted return windows. Even so, the design cannot prove broad causal effects.

Events may be anticipated, bundled with other information, affected by earnings or macro conditions, or influenced by sector-wide cycles. The project therefore treats findings as evidence about market interpretation around selected events, not as definitive causal proof.
...
Several variables are qualitative, including event family, state-support signal, pressure signal, surprise level, market interpretation, and observed pathway. These fields are useful for transparent comparison, but they depend on researcher judgment.

Future expansion should preserve source notes, coding rules, change logs, and explicit handling of `TBD` or `Not coded` values.

## Dashboard Limits

The dashboard is a static evidence view. It displays generated outputs but does not perform independent validation, causal analysis, or forecasting.

If the underlying JSON files are stale or missing, the dashboard can only report the available evidence-file status.
