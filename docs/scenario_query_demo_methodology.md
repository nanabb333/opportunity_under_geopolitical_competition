...
The scenario query demo shows how the historical analog dataset and similarity logic can answer example scenario questions through deterministic historical retrieval. It is a portfolio demonstration of structured analog reasoning, not a forecasting model.

The script uses three hard-coded questions and maps each one to a coded scenario profile. The profile is then compared against every event in `data/historical_analog_events.csv`.
...
The final score is the total field score divided by the eight compared fields. The script returns the top three historical analogs for each scenario and includes:

- similarity score;
- matched fields;
- different fields;
- observed market pathway;
- evidence note from the historical analog dataset.
...
The output is evidence-based because each analog comes from the documented historical event dataset. It remains limited because the dataset is small, qualitative, and focused on semiconductor-related geopolitical and industrial-policy cases.
...
The demo should be treated as a transparent proof of concept for analog retrieval, not as a decision engine for policy, markets, or investment.
